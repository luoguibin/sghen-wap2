  ...
  <div class="peotry" item-type="peotry">
    ...
    <div :class="{ wrapper: true, 'avatar-visible': showAvatar }">
      ...
      <div class="peot--time">
        ...
        <span
          ><i class="iconfont icon-history"></i>{{ getTime(peotry.time) }}</span
        >
      </div>
      ...
      <div
        ref="contentEnd"
        :class="{ 'content--end': true, 'max-height': !isExpand }"
      >
        <div
          item-type="peotry-content"
          class="content"
          v-html="safeContent"
        ></div>
...
import { imgSrcFilter, timeFilter } from "@/filters/index";
...
export default {
  ...
  methods: {
    ...
    getAvatarUrl(user = {}) {
      return imgSrcFilter(user.avatar, "avatar");
    },
    getTime(time) {
