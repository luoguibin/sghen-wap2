<template>
  <div class="comments" v-show="currentPraises.length || textComments.length">
    ...
    <div
      v-show="currentPraises.length && textComments.length"
    ...
    <div class="contents" v-show="textComments.length">
      <div
        v-for="comment in textComments"
        class="comment"
        :key="comment.id"
        item-type="comment"
      >
        <span class="names">
          <span class="name" item-type="comment-from">{{
            comment.fromPeot ? comment.fromPeot.username : comment.fromId
          }}</span>
  ...
  computed: {
    ...
    currentPraises() {
      return this.praises.map((o) => {
        const temp = { ...o };
        const peot = o.fromPeot || {};
        temp.avatar = this.isIntoView
          ? imgSrcFilter(peot.avatar, "avatar")
          : defaultAvatarSrc;
        return temp;
      });
    },
