 ...
 * @returns 
 */
export const getSmallImage = function (v: String) {
 ...
 * @returns 
 */
export const resizeImage = function (src: string, maxSize = 800 * 600) {
 ...
 */
export const base64ToFile = function (base64Data: String, fileName: String) {
  const [v0, v1] = base64Data.split(',')
  const vtypes = v0.match(/:(.*?);/) || []
  const fileType = vtypes[1]
  const bstr = atob(v1)
  const tempArr = []
